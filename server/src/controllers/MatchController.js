...
const MatchController = {
  ...
  async join(req, res) {
    const id = parseInt(req.params.id);
    const match = await MatchService.joinMatch(id, req.user.userId);
    res.json(success(match, '报名成功'));
  },
  ...
  async favorite(req, res) {
    ...
    await db.query(
      'INSERT IGNORE INTO match_favorites (user_id, match_id) VALUES (?, ?)',
  ...
  async unfavorite(req, res) {
    ...
    const db = require('../config/database');
    await db.query(
      'DELETE FROM match_favorites WHERE user_id = ? AND match_id = ?',
      [req.user.userId, matchId]
    );
  ...
  async myFavorites(req, res) {
    ...
    const [countResult] = await db.query(
      'SELECT COUNT(*) as total FROM match_favorites WHERE user_id = ?',
      [req.user.userId]
    );
