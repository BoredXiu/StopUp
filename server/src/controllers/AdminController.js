...
const AdminController = {
	async login(req, res) {
		...
		await db.query("UPDATE admin_users SET last_login_at = NOW(), last_login_ip = ? WHERE id = ?", [req.ip, admin.id]);

		const token = jwt.sign({ userId: admin.id, isAdmin: true, role: admin.role }, jwtConfig.secret, { expiresIn: jwtConfig.expiresIn });
	...
	async dashboard(req, res) {
		...
		const [[feedbackCount]] = await db.query("SELECT COUNT(*) as total FROM feedbacks WHERE status = 1");
		const [[todayMatchCount]] = await db.query("SELECT COUNT(*) as total FROM matches WHERE match_date = CURDATE()");

		const [[totalRevenue]] = await db.query("SELECT COALESCE(SUM(amount), 0) as total FROM orders WHERE status IN (2, 4)");
