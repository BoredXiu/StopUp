...
const matchSchema = Joi.object({
	...
	min_players: Joi.number().integer().min(2).optional().allow(0, null, ""),
	fee_type: Joi.number().integer().valid(1, 2, 3).required().messages({
		"any.required": "费用模式为必填项",
	}),
	total_fee: Joi.number().min(0).optional().allow(null, ""),
	per_person_fee: Joi.number().min(0).optional().allow(null, ""),
