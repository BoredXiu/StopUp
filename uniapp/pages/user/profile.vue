<template>
  <view class="page">
    <view class="profile-header" v-if="userStore.user">
      <image :src="userStore.user.avatar || '/static/default-avatar.png'" class="avatar" mode="aspectFill" />
    ...
    <view class="menu-section">
      <view class="menu-item" @tap="goPage('/pages/user/matches')">
      ...
      <view class="menu-item" @tap="goPage('/pages/user/credit')">
        <text class="menu-icon">⭐</text>
        <text class="menu-text">信用记录</text>
        <text class="menu-arrow">→</text>
      </view>
      <view class="menu-item" @tap="goPage('/pages/user/notifications')">
        <text class="menu-icon">🔔</text>
    ...
    <view class="menu-section">
      <view class="menu-item" @tap="goPage('/pages/match/create')">
        <text class="menu-icon">➕</text>
...
.profile-info { flex: 1; }
.nickname { font-size: 18px; font-weight: 700; display: block; }
.bio { font-size: 13px; color: #999; display: block; margin: 4px 0; }
.meta { display: flex; gap: 12px; font-size: 12px; color: #999; }

.menu-section { background: #fff; border-radius: 12px; margin-bottom: 12px; overflow: hidden; }
.menu-item {
  display: flex;
  align-items: center;
  padding: 16px;
...
.login-tip { margin-top: 40px; text-align: center; }
.login-btn {
  width: 200px;
  height: 44px;
  line-height: 44px;
