<template>
  <view class="page">
    <view class="notify-list">
      ...
      >
        <view class="notify-icon">
          <text v-if="item.type === 'match_join'" class="icon-join">👥</text>
          <text v-else-if="item.type === 'match_cancel'" class="icon-cancel">❌</text>
          <text v-else-if="item.type === 'payment'" class="icon-pay">💰</text>
    ...
    <view class="empty" v-if="!loading && notifications.length === 0">暂无通知</view>
  </view>
</template>
...
async function handleClick(item) {
  ...
  if (item.relatedType === 'match' && item.relatedId) {
    uni.navigateTo({ url: `/pages/match/detail?id=${item.relatedId}` })
  }
}
...
.notify-icon { margin-right: 12px; font-size: 22px; }
.notify-content { flex: 1; }
.notify-title { font-size: 14px; font-weight: 600; display: block; }
.notify-text { font-size: 13px; color: #666; display: block; margin: 4px 0; }
