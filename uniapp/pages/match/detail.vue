...
<script setup>
	...
	const statusText = computed(() => {
		...
		return map[match.value && match.value.status] || "未知";
	});
	const statusClass = computed(() => {
		const map = { 1: "open", 2: "full", 3: "started", 4: "ended", 5: "cancelled" };
		return map[match.value && match.value.status] || "";
	});
	const levelText = computed(() => {
		const map = { 1: "入门", 2: "初级", 3: "中级", 4: "高级", 5: "专业" };
		return map[match.value && match.value.levelRequired] || "不限";
	});
	const isCreator = computed(() => (match.value && match.value.creatorId) === (userStore.user && userStore.user.id));
	const isJoined = computed(() => members.value.some((m) => m.id === (userStore.user && userStore.user.id)));
