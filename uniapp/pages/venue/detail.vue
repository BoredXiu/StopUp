<template>
  <view class="page">
    <view class="venue-header" v-if="venue">
      <image :src="venue.coverImage || '/static/placeholder.jpg'" class="cover-image" mode="aspectFill" />
    ...
    <view class="info-card" v-if="venue">
      <view class="info-row">
    ...
    <view class="section">
      ...
      <view class="match-list">
        <view v-for="match in matches" :key="match.id" class="match-item" @tap="goMatch(match.id)">
      ...
      <view class="empty" v-if="!matches.length">暂无球局</view>
    </view>
  </view>
</template>
...
async function fetchDetail() {
  ...
  if (!id) return
  try {
    const [venueRes, matchRes] = await Promise.all([
      venueApi.detail(id),
      matchApi.list({ venueId: id, pageSize: 10, status: 1 }),
    ])
...
.venue-basic { padding: 16px; background: #fff; display: flex; justify-content: space-between; align-items: center; }
.venue-name { font-size: 18px; font-weight: 700; }
.venue-rating { font-size: 14px; color: #E6A23C; }
...
.info-value { font-size: 14px; flex: 1; text-align: right; }
...
.facility-tag { padding: 4px 12px; background: #f0f7ff; color: #409EFF; border-radius: 12px; font-size: 12px; }

.match-list { display: flex; flex-direction: column; }
.match-item { display: flex; justify-content: space-between; align-items: center; padding: 10px 0; border-bottom: 1px solid #f5f5f5; }
.match-item:last-child { border-bottom: none; }
.match-title { font-size: 14px; font-weight: 500; display: block; }
.match-meta { font-size: 12px; color: #999; }
.match-count { font-size: 16px; font-weight: 700; color: #409EFF; }
